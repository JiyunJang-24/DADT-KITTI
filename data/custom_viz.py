...
for filename in filenames:
    ...
    with open(label_file, 'r') as file:
        ...
        if boolean:
            ...
            boxes3d_lidar = boxes3d_lidar[boxes3d_lidar[:, 0] <= 80]
            boxes3d_lidar = boxes3d_lidar[boxes3d_lidar[:, 0] >= 0]
            for i in range(boxes3d_lidar.shape[0]):
                for j in range(max_x//10):
                    if(check[j] == 0 and boxes3d_lidar[i][0] < (j+1)*10.0 and boxes3d_lidar[i][0] > j*10.0):
                        check[j] = 1
                        boxes3d_lidar_condition = np.vstack((boxes3d_lidar_condition, boxes3d_lidar[i]))
                        max_ += 1
                        if max_ == 1:
                            boxes3d_lidar_condition = boxes3d_lidar_condition[1:]
            pts = np.vstack((pts, remove_points_in_boxes3d(pointcloud1, boxes3d_lidar_condition)))
