...
def cal_bev_attention_loss2_no_roi_add_clamp(batch_teacher, batch, index = None):
    """Compute loss for model. If both `labels` and `mask` are None,
    it degenerates to SimCLR unsupervised loss:
    https://arxiv.org/pdf/2002.05709.pdf
...
def cal_bev_attention_loss2_no_roi_add_normalize(batch_teacher, batch, index = None):
    """Compute loss for model. If both `labels` and `mask` are None,
    it degenerates to SimCLR unsupervised loss:
    https://arxiv.org/pdf/2002.05709.pdf
...
def cal_bev_attention_loss2_no_roi_add_softmax(batch_teacher, batch, index = None):
    """Compute loss for model. If both `labels` and `mask` are None,
    it degenerates to SimCLR unsupervised loss:
    https://arxiv.org/pdf/2002.05709.pdf
    ...
    for i in range(batch_size):
        ...
        for label in batch_class_features_dict[i].keys():
            ...
            # kq_cross = F.normalize(torch.matmul(class_teacher_key, student_features_batch.reshape(channel, -1)), dim=0).cuda()
            kq_cross = torch.matmul(class_teacher_key, student_features_batch.reshape(channel, -1)).cuda()
            kq_cross_mean = torch.mean(kq_cross, dim=0) / (class_teacher_key.shape[1] ** 0.5)
            #kq_cross_mean = kq_cross_mean.view(height, width)
            #import pdb; pdb.set_trace()
            kq_cross_softmax = torch.nn.functional.softmax(kq_cross_mean, dim=0)
            kq_cross_softmax = kq_cross_softmax.view(height, width)
            #import pdb; pdb.set_trace()
            kqv_cross = kq_cross_softmax[np.newaxis, :, :] * teacher_features_batch
            #import pdb; pdb.set_trace()
            
            # kq_self = F.normalize(torch.matmul(class_key, student_features_batch.reshape(channel, -1)), dim=0).cuda()
            kq_self = torch.matmul(class_key, student_features_batch.reshape(channel, -1)).cuda()
            kq_self_mean = torch.mean(kq_self, dim=0) / (class_key.shape[1] ** 0.5)
            #kq_self_mean = kq_self_mean.view(height, width)
            
            kq_self_softmax = torch.nn.functional.softmax(kq_self_mean, dim=0)
            kq_self_softmax = kq_self_softmax.view(height, width)
            #import pdb; pdb.set_trace()
            kqv_self = kq_self_softmax[np.newaxis, :, :] * student_features_batch
            
            mse_loss = 0
            weight = 1
            squared_errors = ((kqv_cross - kqv_self)*weight)**2
            #squared_errors2 = ((kq_cross - kq_self)*weight)**2
            mse = torch.sum(squared_errors)
            #mse2 = torch.mean(squared_errors2)
            #mse2 /= class_key.shape[0]
            loss += mse
            #loss += mse2
...
def cal_bev_attention_loss2_no_roi_add_norm_self_v2(batch_teacher, batch, index = None):
    """Compute loss for model. If both `labels` and `mask` are None,
    it degenerates to SimCLR unsupervised loss:
    https://arxiv.org/pdf/2002.05709.pdf
    ...
    for i in range(batch_size):
        # Iterate over rois for the current instance
        student_features_batch = student_features[i]
        student_features_batch_no_channel = torch.norm(student_features_batch, p=2, dim=0)
        teacher_features_batch = teacher_features[i]
        teacher_features_batch_no_channel = torch.norm(teacher_features_batch, p=2, dim=0)
        for j in range(roi_size):
            # Get the current roi's class label
            class_label = int(gt_class[i, j].item())
            ...
            if x1_pixel == x2_pixel and y1_pixel == y2_pixel:
                region_feature = student_features_batch_no_channel[y1_pixel, x2_pixel]
                #region_feature = student_features[i, :, y1_pixel, x2_pixel].unsqueeze(1).unsqueeze(1)
                region_feature_teacher = teacher_features_batch_no_channel[y1_pixel, x2_pixel].unsqueeze(1).unsqueeze(1)
            elif x1_pixel != x2_pixel and y1_pixel == y2_pixel:
                region_feature = student_features_batch_no_channel[y1_pixel, x1_pixel:x2_pixel].unsqueeze(1)
                region_feature_teacher = teacher_features_batch_no_channel[y1_pixel, x1_pixel:x2_pixel].unsqueeze(1)
            elif x1_pixel == x2_pixel and y1_pixel != y2_pixel:
                region_feature = student_features_batch_no_channel[y1_pixel:y2_pixel, x2_pixel].unsqueeze(2)
                region_feature_teacher = teacher_features_batch_no_channel[y1_pixel:y2_pixel, x2_pixel].unsqueeze(2)
            else:
                region_feature = student_features_batch_no_channel[y1_pixel:y2_pixel, x1_pixel:x2_pixel]
                region_feature_teacher = teacher_features_batch_no_channel[y1_pixel:y2_pixel, x1_pixel:x2_pixel]
        ...
        for label in batch_class_features_dict[i].keys():
            ...
            # kq_cross = F.normalize(torch.matmul(class_teacher_key, student_features_batch.reshape(channel, -1)), dim=0).cuda()
            class_teacher_key = class_teacher_key.unsqueeze(1).unsqueeze(2).expand(-1, height, width)
...
def train_one_epoch(model, model_teacher, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    rank, tbar, total_it_each_epoch, dataloader_iter, tb_log=None, leave_pbar=False, mimic_weight=1.0, mimic_mode='roi'):
    ...
    for cur_it in range(total_it_each_epoch):
        ...
        model_teacher.eval()
        optimizer.zero_grad()
        #projector_dict = model.roi_head.state_dict()

        # Load the state_dict into model.roi_head_iou_layers
        #model_teacher.roi_head.load_state_dict(projector_dict)
        ...
        # for i in range(4):
        #loss_attention2_no_roi = cal_bev_attention_loss2_no_roi_add_norm_self(batch_teacher_new, batch_new)
        #loss_attention2_no_roi = cal_bev_attention_loss2_no_roi_add_normalize(batch_teacher_new, batch_new)
        ...
        loss_sum = loss + loss_attention2_no_roi + loss_mimic2 #+ loss_mimic2#* 2.0
        #loss_sum = loss + loss_mimic + loss_mimic2#* 2.0
        ...
        #disp_dict.update({'loss': loss.item(), 'loss_attention': loss_attention2_no_roi.item(), 'loss_mimic': loss_mimic.item(), 'loss_mimic2': loss_mimic2.item(), 'lr': cur_lr})
