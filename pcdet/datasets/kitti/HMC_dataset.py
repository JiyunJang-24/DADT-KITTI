...
class HMCDataset(DatasetTemplate):
    ...
    """
    def __init__(self, dataset_cfg, class_names, training=True, use_ori=False, teacher_tag=None, root_path=None, logger=None):
        """
        Args:
            root_path:
            dataset_cfg:
        ...
        if 'DATA_SAMPLE_NUM' in self.dataset_cfg:
            if 'DATA_SIZE' in self.dataset_cfg:
                split_dir = self.root_path / 'ImageSets' / self.dataset_cfg['DATA_SIZE'] / ('sample' + self.dataset_cfg['DATA_SAMPLE_NUM']) / (self.split + '.txt')
            else:
                split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
        else:
            if 'DATA_SIZE' in self.dataset_cfg:
                split_dir = self.root_path / 'ImageSets' / self.dataset_cfg['DATA_SIZE'] /(self.split + '.txt')
            else:
                split_dir = self.root_path / 'ImageSets' / (self.split + '.txt')
    ...
    def include_kitti_data(self, mode):
            ...
            if self.oss_path is None:
                ...
                with open(info_path, 'rb') as f:
                    ...
                    if 'DATA_SIZE' in self.dataset_cfg:
                        info_change_size = []
                        for info in infos:
                            if info['point_cloud']['lidar_idx'] in self.sample_id_list:
                                info_change_size.append(info)
                        kitti_infos.extend(info_change_size)
                    else: kitti_infos.extend(infos)
            else:
        ...
        '''
        if mode =='test' and self.dataset_cfg['DATA_SIZE'] is not None:
        ...
        if self.logger is not None:
            self.logger.info('Total samples for HMC dataset: %d' % (len(kitti_infos)))
    ...
    def get_lidar(self, idx):
        ...
        if self.oss_path is None:
            if self.use_ori:
                lidar_file = self.root_split_path / 'modes' / self.teacher_tag / ('%s.bin' % idx)
                
            elif self.use_ori is False and self.dataset_cfg.get('LIDAR_DISTILLATION', None): #and self.training:
                lidar_file = self.root_split_path / 'modes' / str(self.dataset_cfg['LIDAR_DISTILLATION'][0]) / ('%s.bin' % idx)
            ...
            if self.use_ori:
                lidar_file = self.root_split_path / 'modes' / self.teacher_tag / ('%s.bin' % idx)
            elif self.use_ori is False and self.dataset_cfg.get('LIDAR_DISTILLATION', None):
                lidar_file = self.root_split_path / 'modes' / self.dataset_cfg['LIDAR_DISTILLATION'][0] / ('%s.bin' % idx)
            else:
                lidar_file = self.root_split_path / 'velodyne' / ('%s.bin' % idx)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = HMCDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
