...
class HanriverDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, class_names, training=True, root_path=None, logger=None):
        ...
        if self.oss_path is not None:
            ...
            self.root_split_path = self.root_path / 'bin'
    ...
    def get_lidar(self, idx):
        if self.oss_path is None:
            lidar_file = self.root_split_path / ('frm_%s.bin' % idx)
            assert lidar_file.exists()
            points = np.fromfile(str(lidar_file), dtype=np.float32).reshape(-1, 4)
        else:
            lidar_file = os.path.join(self.root_split_path, ('frm_%s.bin' % idx))
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    dataset = HanriverDataset(dataset_cfg=dataset_cfg, class_names=class_names, root_path=data_path, training=False)
