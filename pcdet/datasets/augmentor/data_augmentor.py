...
class DataAugmentor(object):
    def __init__(self, root_path, augmentor_configs, class_names, logger=None, oss_flag=False, training=True):
        ...
        if self.training is False:
            for cur_cfg in aug_config_list:
                if not isinstance(augmentor_configs, list):
                    ...
                    if cur_cfg.NAME == 'world_z_flatten':
    ...
    def world_z_flatten(self, data_dict=None, config=None):
        if data_dict is None:
            return partial(self.world_z_flatten, config=config)
        #import pdb; pdb.set_trace()
        
        model = RANSACRegressor()
        model.fit(data_dict['points'][:, :2], data_dict['points'][:, 2])

        # Extract ground points
        ground_mask = model.inlier_mask_
        ground_points = copy.deepcopy(data_dict['points'][ground_mask])
        
        x = ground_points[:, 0].reshape(-1, 1)
        z = ground_points[:, 2].reshape(-1, 1)
        linear = LinearRegression().fit(x, z)
        coef_x, intercept = linear.coef_[0], linear.intercept_
        #data_dict['points'][:, 2] = data_dict['points'][:, 2] - (coef_x*data_dict['points'][:, 0] + intercept)
        #data_dict['gt_boxes'][:, 2] = data_dict['gt_boxes'][:, 2] - (coef_x*data_dict['gt_boxes'][:, 0] + intercept)
        
        rot_range = -1* math.atan(coef_x)
        
        gt_boxes, points = augmentor_utils.global_rotation(
            data_dict['gt_boxes'], data_dict['points'], rot_range=rot_range, flatten=True 
        )
    ...
    def forward(self, data_dict):
        ...
        Returns:
        """
        if 'training' in data_dict and data_dict['training'] == False:
