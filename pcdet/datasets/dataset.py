...
class DatasetTemplate(torch_data.Dataset):
    def __init__(self, dataset_cfg=None, class_names=None, training=True, root_path=None, use_ori=False, logger=None):
        super().__init__()
        ...
        if self.oss_path is not None:
            ...
            if self.training is False:
                self.data_augmentor = DataAugmentor(
                    self.root_path, self.dataset_cfg.DATA_AUGMENTOR, self.class_names, logger=self.logger, oss_flag=False, training=False
                )
    ...
    def fill_pseudo_labels(self, input_dict):
        ...
        for i in range(len(self.class_names)):
            dict1.append(self.class_names[i])
        dict1 += ['other']
        list1 = []
        for i in gt_classes:
            list1.append(dict1[int(i)])
        
        gt_names = np.array(list1)
    ...
    def collate_batch(batch_list, _unused=False):
        if type(batch_list[0]) == tuple:
            process_list = [[data[0] for data in batch_list], [data[1] for data in batch_list]]
            flag = True
        else:
            process_list = [batch_list]
            flag = False
        ret_list = []
        for batch_list_ in process_list:
            data_dict = defaultdict(list)
            for cur_sample in batch_list_:
                #assert 'points'  in cur_sample.keys()
                #print(cur_sample['points'].shape, cur_sample['frame_id'])
                for key, val in cur_sample.items():
                    data_dict[key].append(val)
            batch_size = len(batch_list_)
            ret = {}
    ...
    @staticmethod
    def collate_batch_lidar_distillation(batch_list, _unused=False):
        if type(batch_list[0]) == tuple:
            process_list = [[data[0] for data in batch_list], [data[1] for data in batch_list]]
            flag = True
        else:
