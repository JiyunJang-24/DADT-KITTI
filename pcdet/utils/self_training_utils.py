    ...
    for b_idx in range(batch_size):
        ...
        if 'pred_boxes' in pred_dicts[b_idx]:
            ...
            if cfg.SELF_TRAIN.get('NEG_THRESH', None):
                ...
                new_list = [0] * len(pred_labels)
                #labels_remove_scores = np.array(cfg.SELF_TRAIN.NEG_THRESH)[pred_labels - 1]
                labels_remove_scores = np.array(cfg.SELF_TRAIN.NEG_THRESH)[new_list]
            ...
            new_list = [0] * len(pred_labels)
            labels_ignore_scores = np.array(cfg.SELF_TRAIN.SCORE_THRESH)[new_list]
